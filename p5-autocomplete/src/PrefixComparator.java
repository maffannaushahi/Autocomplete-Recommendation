...
public class PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        String word1 = v.getWord(); //initialize strings getting compared
        String word2 = w.getWord();
        for(int i=0; i < myPrefixSize; i++){
            if(i == word1.length() || i == word2.length()){ 
                return word1.length()-word2.length(); //return smaller prefix
            }
            if(word1.charAt(i)==word2.charAt(i)){
                continue;
            }
            return word1.charAt(i) - word2.charAt(i); // compare both terms 
        }
        return 0;
    }
}
